...
export function maybeAddNamedImport(
  ...
  const importFromRightModule = importStatements
    ...
    .filter((binding): binding is NamedImportsFromModule =>
...
export function maybeAddNamespaceImport(
  ...
  const hasTheRightImport = importStatements
    ...
    .filter((binding): binding is NamespaceImportFromModule =>
      ts.isNamespaceImport(binding.namedBindings),
    )
    .some(
      (binding) =>
        binding.fromModule === fromModule &&
 ...
 */
function maybeParseImportNode(
  iDecl: ts.ImportDeclaration,
